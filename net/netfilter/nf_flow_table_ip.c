...
static int nf_flow_neigh_xmit_list(struct sk_buff *skb, struct net_device *outdev, const void *daddr)
{
	...
	if (nft_dev_queue_xmit(skb) != NET_XMIT_SUCCESS)
		return -1;
...
static void ip_dbg(struct sk_buff *skb)
{
...
//		}
	...
	list_for_each_entry_safe(skb, n, bulk_head, list) {
		...
		sent = nf_flow_neigh_xmit_list(skb, rt->dst.dev, neigh->ha);
		nBulk += sent;

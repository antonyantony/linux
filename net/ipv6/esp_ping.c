...
static int esp_ping_v6_pre_connect(struct sock *sk, struct sockaddr *uaddr,
			       int addr_len)
{
...
static int esp_ping_v6_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	err = esp_ping_common_sendmsg(AF_INET6, msg, len, &user_icmph,
				  sizeof(user_icmph));
	if (err)
		return err;
	...
	lock_sock(sk);
	err = ip6_append_data(sk, esp_ping_getfrag, &pfh, len,
...
EXPORT_SYMBOL_GPL(esp_pingv6_prot);

static struct inet_protosw esp_pingv6_protosw = {
	.type =      SOCK_DGRAM,
	.protocol =  IPPROTO_ICMPV6,
	.prot =      &esp_pingv6_prot,
...
static int esp_ping_v6_seq_show(struct seq_file *seq, void *v)
{
	if (v == SEQ_START_TOKEN) {
		...
		int bucket = ((struct esp_ping_iter_state *) seq->private)->bucket;
...
{
	if (!proc_create_net("icmp6", 0444, net->proc_net, &esp_ping_v6_seq_ops,
...
int __init esp_pingv6_init(void)
{
	...
	if (ret)
		return ret;
#endif
	esp_pingv6_ops.ipv6_recv_error = ipv6_recv_error;
	...
	esp_pingv6_ops.icmpv6_err_convert = icmpv6_err_convert;
	esp_pingv6_ops.ipv6_icmp_error = ipv6_icmp_error;
	esp_pingv6_ops.ipv6_chk_addr = ipv6_chk_addr;
	return inet6_register_protosw(&esp_pingv6_protosw);
}
...
{
	esp_pingv6_ops.ipv6_recv_error = dummy_ipv6_recv_error;
	...
	esp_pingv6_ops.icmpv6_err_convert = dummy_icmpv6_err_convert;
	esp_pingv6_ops.ipv6_icmp_error = dummy_ipv6_icmp_error;
	esp_pingv6_ops.ipv6_chk_addr = dummy_ipv6_chk_addr;
#ifdef CONFIG_PROC_FS
	unregister_pernet_subsys(&esp_ping_v6_net_ops);
#endif
	inet6_unregister_protosw(&esp_pingv6_protosw);
}

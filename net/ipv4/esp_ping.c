...
struct esp_pingv6_ops esp_v6_ops;
...
int esp_ping_get_port(struct sock *sk, unsigned short ident)
{
	...
	if (ident == 0) {
		...
		for (i = 0; i < (1L << 16); i++, result++) {
			...
			/* found */
			esp_ping_port_rover = ident = result;
			break;
next_port:
			;
		}
...
void esp_ping_unhash(struct sock *sk)
{
	...
	pr_debug("esp_ping_unhash(isk=%p,isk->num=%u)\n", isk, isk->inet_num);
	spin_lock(&esp_ping_table.lock);
...
{
	pr_debug("esp_ping_close(sk=%p,sk->num=%u)\n",
...
static int esp_ping_pre_connect(struct sock *sk, struct sockaddr *uaddr,
			    int addr_len)
{
				...
				struct sockaddr *uaddr, int addr_len)
{
	...
	if (sk->sk_family == AF_INET) {
		...
		pr_debug("esp_ping_check_bind_addr(sk=%p,addr=%pI4,port=%d)\n",
		...
		pr_debug("esp_ping_check_bind_addr(sk=%p,addr=%pI6c,port=%d)\n",
...
static void esp_ping_set_saddr(struct sock *sk, struct sockaddr *saddr)
{
...
static inline int esp_ping_supported(int family, int type, int code)
{
...
void esp_ping_err(struct sk_buff *skb, int offset, u32 info)
{
	...
	if (!esp_ping_supported(family, icmph->type, icmph->code))
		return;

	pr_debug("%s(proto=0x%x,type=%d,code=%d,id=%04x,seq=%04x)\n", __func__,
		 skb->protocol, type, code, ntohs(icmph->un.echo.id),
		 ntohs(icmph->un.echo.sequence));

	sk = esp_ping_lookup(net, skb, ntohs(icmph->un.echo.id));
	if (!sk) {
		pr_debug("no socket, dropesp_\n");
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		...
		case ICMP_SOURCE_QUENCH:
			/* This is not a real error but esp_ wants to see it.
		...
		harderr = esp_v6_ops.icmpv6_err_convert(type, code, &err);
#endif
	}
	...
	if ((family == AF_INET && !inet_test_bit(RECVERR, sk)) ||
		...
		if (family == AF_INET) {
			...
			esp_v6_ops.ipv6_icmp_error(sk, skb, err, 0,
						   info, (u8 *)icmph);
#endif
		}
	}
...
EXPORT_SYMBOL_GPL(esp_ping_err);
...
int esp_ping_getfrag(void *from, char *to,
		 int offset, int fraglen, int odd, struct sk_buff *skb)
{
	struct esp_pingfakehdr *pfh = from;
	 ...
	 * wcheck, it will be finalized in esp_ping_v4_push_pending_frames.
	 */
...
EXPORT_SYMBOL_GPL(esp_ping_getfrag);

static int esp_ping_v4_push_pending_frames(struct sock *sk, struct esp_pingfakehdr *pfh,
				       struct flowi4 *fl4)
{
			...
			void *user_icmph, size_t icmph_len)
{
	...
	if (!esp_ping_supported(family, type, code))
		return -EINVAL;
...
static int esp_ping_v4_sendmsg(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	pr_debug("esp_ping_v4_sendmsg(sk=%p,sk->num=%u)\n", inet, inet->inet_num);

	err = esp_ping_common_sendmsg(AF_INET, msg, len, &user_icmph,
				      sizeof(user_icmph));
	if (err)
		return err;
...
back_from_confirm:
	...
	err = ip_append_data(sk, &fl4, esp_ping_getfrag, &pfh, len,
			     sizeof(struct icmphdr), &ipc, &rt,
			     msg->msg_flags);
	if (err)
		ip_flush_pending_frames(sk);
	else
		err = esp_ping_v4_push_pending_frames(sk, &pfh, &fl4);
	release_sock(sk);
		     ...
		     int *addr_len)
{
	...
	if (family == AF_INET) {
		...
		if (inet6_sk(sk)->rxopt.all)
			esp_v6_ops.ip6_datagram_recv_common_ctl(sk, msg, skb);
		if (skb->protocol == htons(ETH_P_IPV6) &&
		    inet6_sk(sk)->rxopt.all)
			esp_v6_ops.ip6_datagram_recv_specific_ctl(sk, msg, skb);
...
enum skb_drop_reason esp_ping_rcv(struct sk_buff *skb)
{
	...
	pr_debug("%s(skb=%p,id=%04x,seq=%04x)\n", __func__,

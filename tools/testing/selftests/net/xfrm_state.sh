...
tests="
	...
	mark_wildcard_add_blocked	mark: match-all SA blocks adding a specific-mark SA
	...
	mark_exact_unmasked_delete	mark: exact-match delete tolerates unmasked query value
...
run_test() {
	...
	(
		...
		mark_wildcard_add_blocked) test_mark_wildcard_add_blocked ;;
		mark_wildcard_shadow)      test_mark_wildcard_shadow ;;
		mark_wildcard_delete)      test_mark_wildcard_delete ;;
		mark_exact_unmasked_delete) test_mark_exact_unmasked_delete ;;
...
# A match-all SA (mark 0/0) already claims every mark value; adding a
# specific-mark SA at the same tuple afterward must fail EEXIST.

test_mark_wildcard_add_blocked() {
	...
	run_cmd_err "${ns_a[@]}" ip xfrm state add \
		src "${src}" dst "${dst}" proto esp spi 0x1000 \
		reqid 100 mode tunnel \
		aead 'rfc4106(gcm(aes))' 0x1111111111111111111111111111111111111111 96 \
		mark 0x2 mask 0xffffffff
	[ "${rc}" -ne 0 ] || result=1
...
test_mark_exact_unmasked_delete() {
	setup ns_set_simple namespaces || return "$ksft_skip"
	local result=0
